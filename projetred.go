...
func main() {
	var p1 Perso
	fmt.Println("Bienvenue Sur For Honor En Golang!")
	p1.Init("oroshi", "\nSamurai", []string{" Sabre, bouclier"}, 1, 1000, 150, []string{"coup de poing"}, 100)
	fmt.Println("===============")
	fmt.Println("Personnage 1 :\nNom :", p1.nom, "\nClasse :", p1.classe)
	fmt.Println("===============")
	var p2 Perso
	p2.Init("raider", "\nviking", []string{" Hache,", "Potion de soin"}, 1, 1500, 300, []string{"coup de poing"}, 100)
	...
	if choice == "1" {
		...
		pc.Init(p2.nom, p2.classe, p2.inv, p2.niveau, p2.pv, p2.pva, p2.skill, p2.coins)
		fmt.Println(pc)
	} else {
		fmt.Println("Veuillez tapez 1 ou 2")
	}
	fmt.Println("Vous avez choisi le personnage :")
	fmt.Println("Nom :", pc.nom, "\nClasse :", pc.classe, "skill:", pc.skill, pc.coins)
	fmt.Println("===============")
	fmt.Println("Menu")
	fmt.Println("===============")
...
func (p *Perso) marchand() {
	...
	fmt.Println("3 : potion de poison (250 coins)")
	if choice == 3 {
		if p.coins<=250{
